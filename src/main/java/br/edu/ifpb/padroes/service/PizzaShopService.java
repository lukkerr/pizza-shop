...
public class PizzaShopService {
    ...
    public List<Pizza> getPizzas() {
    	return Stream.concat( getPizzasPizzaHot(), getPizzasDamenos() ).toList();
    }

    public Stream<Pizza> getPizzasDamenos() {
        return damenosService.getPizzas().stream().map(p -> new DamenosAdapter(p));
    }

    public Stream<Pizza> getPizzasPizzaHot() {
        return pizzaHotService.getPizzas().stream().map(p -> new PizzahotAdapter(p));
    }
